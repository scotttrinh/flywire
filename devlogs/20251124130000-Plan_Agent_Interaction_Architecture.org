...
- Provide shared test helpers/macros (either in an existing `test/*.el` or a new `test/flywire-test-helpers.el`):
  - `(flywire-test-with-session env &rest body)` — macro that:
    - Creates a `flywire-session` with the given environment (or env builder function).
    - Binds a local `session` variable for use in `body`.
    - Ensures any environment `teardown` is called at the end of the test.
  - `(flywire-test-with-headless-session &rest body)` — convenience macro using `flywire-test-env-headless` and binding `session`.
  - `(flywire-test-collect-events session thunk)` — helper that:
    - Temporarily registers an event handler on `session`.
    - Runs `thunk`.
    - Returns the list of events emitted during `thunk`.
  - `(flywire-test-wait-for-event session predicate &optional timeout)` — helper that:
    - Subscribes to events on `session`.
    - Blocks (using `accept-process-output` / `sleep-for`) until `predicate` returns non-nil for an event or `timeout` elapses.
    - Returns the matching event (or `nil` on timeout).

These helpers let `ert` tests stay small and declarative while still exercising async and environment behavior in a realistic way.
